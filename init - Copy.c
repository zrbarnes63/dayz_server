...
class CustomMission: MissionServer
{
	override void OnInit()
	{
		...
		if ( m_EventManagerServer )
		{
			...
			// m_EventManagerServer.OnInitServer( true, 30, 40, 1 );
	...
	override void StartingEquipSetup( PlayerBase player, bool clothesChosen )
	{
		...
		// customized by fadesfast
		player.GetInventory().CreateInInventory("DryBag_black"); // add drybag
		player.GetInventory().CreateInInventory("compass"); // add compass
		itemEnt = player.GetInventory().CreateInInventory("M4A1"); // add m4
		player.SetQuickBarEntityShortcut(itemEnt, 0); //set m4 to slot 1
		itemEnt.GetInventory().CreateInInventory("AD_ACOG"); // test
		player.GetInventory().CreateInInventory("AmmoBox_556x45_20Rnd"); // add m4 ammo
		player.GetInventory().CreateInInventory("Mag_STANAG_30Rnd"); // add m4 mag
		itemEnt.GetInventory().CreateInInventory("M4_Suppressor"); // M4_Suppressor
		itemEnt.GetInventory().CreateInInventory("M4_MPBttstck"); // M4_MPBttstck
		itemEnt.GetInventory().CreateInInventory("M4_MPHndgrd"); // M4_MPHndgrd
		player.GetInventory().CreateInInventory("PlateCarrierVest");// PlateCarrierVest
		player.GetInventory().CreateInInventory("Poncho_Red_Checkered"); // Poncho_Red_Checkered
	}
};
